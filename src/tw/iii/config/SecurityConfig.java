...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers("/","/register.controller").permitAll()		//.antMatchers("/").permitAll()=>代表此路徑下的網頁不需驗證     .antMatchers("/")可帶多個路徑(api)
		.antMatchers("/loginpass/**").authenticated().anyRequest().permitAll().and().formLogin().loginPage("/login")		//.authenticated()=>存取必須通過驗證		.anyRequest()
